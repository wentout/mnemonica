...
const test = (opts) => {
	...
	describe('Hooks Tests', () => {
		it('check invocations count', () => {
			...
			assert.equal(116, namespaceFlowCheckerInvocations.length);
			assert.equal(114, typesFlowCheckerInvocations.length);
			assert.equal(60, typesPreCreationInvocations.length);
			// there are two errors on creation
			// checked before
			// that is why, and with clones
			assert.equal(54, typesPostCreationInvocations.length);
			assert.equal(61, namespacePreCreationInvocations.length);
			// there are two registered Hooks, that is why
			assert.equal(110, namespacePostCreationInvocations.length);
		});
	});

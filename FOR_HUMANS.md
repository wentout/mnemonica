...
### Nested `lookupTyped()` — resolved as unnecessary

A type-safe `.lookupTyped()` method on constructors (e.g.
`Scene2D.lookupTyped('GraphNode2D')`) was considered so that the returned
...
- Tactica-generated declaration merging already types those instance-level
  subtype constructors. Starting from `lookupTyped('Scene2D')` gives a fully
  typed root constructor; after `const scene = new Scene2D(...)`,
  `scene.GraphNode2D` is typed and produces the correct chained instance.

If `lookupTyped('Something')` resolves to `TypeConstructor<never>`, it means
one of three things:
...
How nested `.lookupTyped()` could have been implemented if it were needed:

1. Encode the type's dotted path in `TypeConstructor` itself:

   ```typescript
   interface TypeConstructor<Instance, Path extends string = string> {
       lookupTyped<const K extends keyof NestedTypeRegistry[Path]>(
           name: K

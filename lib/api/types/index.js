...
const addProps = (inheritedInstance, opts) => {
	...
	odp(inheritedInstance, '__args__', {
		get () {
			return args;
		}
	});
	
	odp(inheritedInstance, '__collection__', {
		get () {
			return collection;
		}
	});
	
	odp(inheritedInstance, '__namespace__', {
		get () {
			return namespace;
		}
	});
	
	odp(inheritedInstance, '__subtypes__', {
		get () {
			return subtypes;
		}
	});
	
	odp(inheritedInstance, '__type__', {
		get () {
			return type;
		}
	});
	
	odp(inheritedInstance, '__parent__', {
		get () {
	...
	odp(inheritedInstance, 'fork', {
		get () {
			const Constructor = isSubType ?
				existentInstance.clone[TypeName] :
					collection[TypeName];
			// const craftingArgs = Object.assign([], args.slice(), forkArgs);
			return function (...forkArgs) {
				const fork = new Constructor(...forkArgs);
				return fork;
			};
		}
	});
		...
		odp(proto, name, {
			...
			get () {
				...
				const SubTypeConstructor = function (...args) {
					...
					const inheritedSubInstance = createInstance
...
const createInstance = function (type, args) {
	...
	addProps(inheritedInstance, opts);

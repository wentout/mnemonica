	...
	odp(inheritedInstance, 'clone', {
		get() {
			const Constructor = isSubType ?
				// inheritedInstance.__parent__[TypeName] :
				existentInstance[TypeName] :
				collection[TypeName];
			const cloned = new Constructor(...args);
			return cloned;
		}
	});
	...
	odp(inheritedInstance, 'fork', {
		get() {
			return function (...forkArgs) {
				...
				let forked;
				// console.log('\n\n1. fork : ', TypeName, inheritedInstance.__args__);

				if (isSubType) {
					
					// const parentParent = existentInstance.__parent__.clone;
					// const parent = new parentParent[
					// 		existentInstance.__type__.TypeName
					// 	](existentInstance.__args__);
					
					// const parent = inheritedInstance.__parent__;
					// forked = createInstance
					// 	.call(parent, parent.__subtypes__[TypeName], forkArgs);
					// // forked = new parent[TypeName](...forkArgs);
					
					forked = new existentInstance[TypeName](...forkArgs);
				} else {
					forked = new collection[TypeName](...forkArgs);
				}
				...
				// console.log('2. fork : ', TypeName, inheritedInstance.__args__);
				// console.log('3. fork : ', TypeName, forked.__args__);
				// console.log('4. fork : ', TypeName, forkArgs, '\n\n');
...
const addSubTypes = (subtypes, inheritedInstance) => {
	...
	const proto = Object.getPrototypeOf(Object.getPrototypeOf(inheritedInstance));
	...
	Object.entries(subtypes).forEach((subtype) => {
		...
		// odp(inheritedInstance.constructor[SymbolConstructorName], name, {
		odp(proto, name, {
			...
			get() {
				...
				const SubTypeConstructor = function (...args) {
					...
					// console.log('ii  |', inheritedInstance.__args__);
					// console.log('isi |', inheritedSubInstance.__args__);
					// console.log('ti  |', inheritedInstance.__timestamp__);
					// console.log('tsi |', inheritedSubInstance.__timestamp__);
...
const Mnemonica = function (namespace, mnemonica = {}) {

	odp(mnemonica, SymbolConstructorName, {
		get() {
	...
	Object.setPrototypeOf(mnemonica, this);
...
const createInstance = function (type, args) {
	...
	const existentInstance = isSubType ? this : base;
	...
	// addProps(InstanceModificator.constructor.prototype, opts);

	const inheritedInstance = proceedInheritance(opts);

	// addProps.call(opts, inheritedInstance);
	addSubTypes(subtypes, inheritedInstance);

	addMethods(inheritedInstance, opts);
...
const defineTypeProxy = function (type) {
	...
	const typeProxyConstructMethod = function (target, args) {
		...
		const instance = createInstance.call(target, type, args);
		return instance;
	};
...
const TypeDescriptor = function (
	...
	const type = Object.assign(this, {
		...
		get base() {
			return new Mnemonica(namespace);
		},

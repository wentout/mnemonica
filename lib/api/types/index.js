...
const makeWaiterProxy = function (waiter, type) {

	const inheritedInstance = this;

	if (!inheritedInstance.chain) {
		const chain = [];

		odp(inheritedInstance, 'chain', {
			get() {
				return chain;
			}
		});

		waiter.then(async (instance) => {
			chain;
			// debugger;
			...
			// }
			// debugger;
			// const params = chain.shift();
			// return await instance[params.TypeName](...params.args);
		});

		inheritedInstance.then(async (instance) => {
			chain;
			// debugger;
			return await instance;
			// if (!chain.length) {
	...
	Object.entries(type.subtypes).forEach((entry) => {
		...
		waiter[name] = (...args) => {
			...
			const result = makeWaiterProxy.call(inheritedInstance, waiter, subtype);

			Promise
				.resolve(result)
				.then((instance) => {
					if (!inheritedInstance.chain.length) {
						// debugger;
						return instance;
					}
					const params = inheritedInstance.chain.shift();
					const {
						TypeName,
						// subtype : _subtype,
						args : _args,
					...
					// addSubTypes(subtype.subtypes, instance);
					// invokePostHooks(instance);
					
					const finished = instance[TypeName](..._args);
	...
	// return new Proxy(waiter, {
	// 	get(proxy, _name) {
	// 		const _subtype = type.subtypes[_name];
	// 		if (_subtype) {
	// 			const _waiter = (..._args) => {
	...
	// 					_subtype,
	// 					_args,
	// 					TypeName: _subtype.TypeName
	// 				});

	// 				return makeWaiterProxy.call(inheritedInstance, _waiter, _subtype);
	// 			};
	// 			return waiter;
	// 			// return makeWaiterProxy.call(inheritedInstance, _waiter, _subtype);
	// 		}
...
const createInstance = function (type, args) {
	...
	if (inheritedInstance instanceof Promise) {
		...
		return makeWaiterProxy.call(inheritedInstance, waiter, type);

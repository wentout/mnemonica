...
const addThen = (inheritedInstance, then) => {
	inheritedInstance = inheritedInstance
		...
		.then(async (instance) => {
			// this both methods done there,
			// inside of makeWaiter
			// so this is just as a memory
			// and for the case
			// addSubTypes(then.subtype.subtypes, instance);
			// invokePostHooks(instance);

			// !!! BUT if our instance is again promise !!!
			// it will wait for resolve ;^)
...
const makeWaiter = function (inheritedInstance, type, then) {

	inheritedInstance = inheritedInstance
		.then(async (instance) => {
			addSubTypes(type.subtypes, instance);
			invokePostHooks(instance);
			return await instance;
		});

...
// The never-guard keeps a fully-covering child clean (just T, no empty Pick).
...
Today `TypeConstructor` is an alias of `_Internal_TC_` — one constructor shape,
both `new`-able and callable:

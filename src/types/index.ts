 ...
 *
 * This interface starts empty (with an index signature that resolves to
 * `TypeConstructor<never>`). Application code or `@mnemonica/tactica` augments
 * it through TypeScript module augmentation:
 *
 * ```ts
 * declare module 'mnemonica' {
 *   interface TypeRegistry {
 *     'MyType': new (...args: unknown[]) => MyTypeInstance;
 *     'MyType.SubType': new (...args: unknown[]) => SubTypeInstance;
 *   }
 * }
 * ```
 *
 * Once augmented, `lookupTyped('MyType')` and the typed utility family
 * (`parentTyped`, etc.) resolve keys against this registry.
 */
export interface TypeRegistry {
	...
	// utilities like lookupTyped() and parentTyped().
}
 ...
 * so utilities like `parentTyped` can derive an instance's path from its
 * type alone.
 */
...
export interface UtilsCollection {
	...
	parentTyped<P extends keyof TypeRegistry>(
		instance: InstanceOfTypeRegistry<ChildKeysOf<P & string>>,
		path: P
	): InstanceOfTypeRegistry<P> | undefined;
